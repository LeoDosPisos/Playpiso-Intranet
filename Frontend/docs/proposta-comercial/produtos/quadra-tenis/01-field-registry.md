...
| `comprimento_alambrado_lateral_1` / `_lateral_2` / `_fundo_1` / `_fundo_2` | Comprimento | number | NumberInput | Sim quando visível | Se `possui_alambrado = true` E `sistema_alambrado = especial` | 4 campos análogos, um por lado. Unidade em metros. |
| `altura_alambrado_lateral_1` / `_lateral_2` / `_fundo_1` / `_fundo_2` | Altura | number | NumberInput | Sim quando visível | Se `possui_alambrado = true` E `sistema_alambrado = especial` | 4 campos análogos, um por lado. Unidade em metros. |
| `espacamento_postes_tubos_lateral_1` / `_lateral_2` / `_fundo_1` / `_fundo_2` | Espaçamento entre postes | number | NumberInput | Sim quando visível | Se `possui_alambrado = true` E `sistema_alambrado = especial` | 4 campos análogos, um por lado. Unidade em metros. |
