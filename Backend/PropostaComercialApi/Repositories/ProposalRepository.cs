...
public class ProposalRepository(string connectionString) : IProposalRepository
{
    ...
    public async Task<IEnumerable<Proposal>> ListAsync(string? createdByUserId = null, string? status = null, int page = 1, int pageSize = 20)
    {
        ...
        if (createdByUserId is not null) where.Add("created_by_user_id = @CreatedByUserId");
        if (status is not null) where.Add("status = @Status");
        var whereClause = where.Count > 0 ? "WHERE " + string.Join(" AND ", where) : "";

        return await conn.QueryAsync<Proposal>(
            $"""
            SELECT * FROM proposals {whereClause}
    ...
    public async Task<bool> UpdateGeneratedOutputsAsync(Guid id, string? pptxUrl, string? xlsxUrl)
    {
        using var conn = Connect();
        var rows = await conn.ExecuteAsync(
            ...
            UPDATE proposals SET
                ...
                updated_at = NOW()
            WHERE id = @Id
            """, new { Id = id, PptxUrl = pptxUrl, XlsxUrl = xlsxUrl });
        return rows > 0;
    }
