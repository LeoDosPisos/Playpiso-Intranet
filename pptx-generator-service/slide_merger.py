...
# Placeholders whose replacement text must be rendered in black, non-bold
_RESET_FORMAT_KEYS = frozenset({'sumario'})
...
SLIDE_FILE_MAP: dict[str, str] = {
    ...
    "alambrado_iluminacao_beach_tenis":   "beach_tenis/alambrado_iluminacao.pptx",
    ...
    "alambrado_iluminacao_quadra_tenis":  "quadra_tenis/alambrado_iluminacao_quadra_tenis.pptx",
    ...
    "acessorios_quadra_poliesportiva":                          "quadra_poli/acessorios.pptx",
    "alambrado_iluminacao_tela_superior_quadra_poliesportiva":  "quadra_poli/alambrado_iluminacao_tela_superior.pptx",
...
CONDITIONAL_SECTIONS: dict[str, list[tuple[str, str]]] = {
    "alambrado_iluminacao_quadra_tenis": [
        ("Alambrado", "possui_alambrado"),
        ("Iluminação", "possui_iluminacao"),
    ],
    "alambrado_iluminacao_beach_tenis": [
        ("Alambrado", "possui_alambrado"),
        ("Iluminação", "possui_iluminacao"),
    ],
    "alambrado_iluminacao_tela_superior_quadra_poliesportiva": [
        ("Alambrado", "possui_alambrado"),
        ("Iluminação", "possui_iluminacao"),
    ],
}
...
def _build_context(global_values: dict, product_groups: list) -> dict:
    ctx = {
        ...
        "sumario":           "\n".join(g.sumarioText for g in product_groups),
    }
    for group in product_groups:
        ...
        if values.get('possui_alambrado'):
            c = values.get('comprimento_alambrado')
            h = values.get('altura_alambrado')
            try:
                ctx['area_alambrado'] = _fmt_numero(float(c) * float(h))
        ...
        # ── sistema alambrado ─────────────────────────────────────────────
        # beach tennis: selecionado pelo usuário (gaiola / trapezio)
        # quadra_tenis: sempre Trapézio (sem campo de seleção no formulário)
        raw_sistema = values.get('sistema_alambrado_beach_tenis', '')
        if raw_sistema:
            ctx['sistema_alambrado'] = _SISTEMA_ALAMBRADO_LABELS.get(raw_sistema, raw_sistema)
        elif group.productId in ('quadra_tenis', 'quadra_poliesportiva'):
            ctx['sistema_alambrado'] = 'Trapézio'
        else:
...
def _replace_in_paragraph(paragraph, context: dict) -> None:
    ...
    for run in paragraph.runs[1:]:
        run.text = ""
    if matched_key in _RESET_FORMAT_KEYS:
